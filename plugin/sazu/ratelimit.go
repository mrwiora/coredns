...
// push re-verifies and re-signs an entire zone's worth of content (and,
// at first contact, walks the chain of trust to the real DNS root),
// while a key-management push never touches served content at all.
const (
...
type RateLimiter struct {
	...
	keyManagement map[string][]time.Time
	now           func() time.Time // overridable in tests
	lastSwept     time.Time
}

// NewRateLimiter returns a RateLimiter enforcing fullPerDay content
// pushes and keyMgmtPerDay key-management pushes per zone, per rolling
// 24h window.
func NewRateLimiter(fullPerDay, keyMgmtPerDay int) *RateLimiter {
	return &RateLimiter{
		fullPerDay:    fullPerDay,
		keyMgmtPerDay: keyMgmtPerDay,
		window:        24 * time.Hour,
		full:          make(map[string][]time.Time),
		keyManagement: make(map[string][]time.Time),
		now:           time.Now,
	}
}

// Allow reports whether a push of the given kind (full content, or
// key-management) for zone is within quota. If so, it records the push
// immediately as part of the same call -- check-and-record atomically
// under one lock, so two concurrent callers can't both observe "still
// room" for what is really only one remaining slot. Call this only for
// a push that has already passed authentication (SIG(0)): a quota is a
// bound on legitimate churn, not an identity check, and counting
// unauthenticated attempts against a zone's own quota would let anyone
// lock a customer out of their own zone with no proof of control over
// it at all.
func (r *RateLimiter) Allow(zone string, full bool) bool {
	r.mu.Lock()
	defer r.mu.Unlock()

	bucket, limit := r.keyManagement, r.keyMgmtPerDay
	if full {
		bucket, limit = r.full, r.fullPerDay
	}
	now := r.now()
	sweepIfDue(&r.lastSwept, r.window, now, r.full, r.keyManagement)
